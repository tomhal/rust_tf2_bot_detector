...
/// RConClient implements an synchronous utf8 Source RCon.
...
impl RConClient {
    ...
    pub fn exec_command(&mut self, cmd: &String) -> BoxResult<String> {
        let request_pkt = Pkt {
            id: self.next_id(),
    ...
    fn write_string(&mut self, value: &String) -> BoxResult<()> {
        self.stream.write_all(value.as_str().as_ref())?;
